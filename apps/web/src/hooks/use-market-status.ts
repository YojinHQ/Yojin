...
import { isUSMarketHoliday } from '@yojin/market-utils';
...
/** Format ET date parts into a YYYY-MM-DD key for holiday lookup. */
...
function compute(): MarketState {
  ...
  // Weekends & holidays (single clock sample — no midnight-boundary race)
  if (day === 'Sat' || day === 'Sun') return { status: 'closed', label: 'Closed' };
  if (isUSMarketHoliday(toDateKey(et))) return { status: 'closed', label: 'Closed' };
