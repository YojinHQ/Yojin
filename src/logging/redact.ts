...
const REDACTION_PATTERNS: Array<{ pattern: RegExp; replacement: string | ((match: string) => string) }> = [
  // Anthropic API keys
  ...
  { pattern: /(?:api[_-]?key|apikey)\s*[:=]\s*["']?[\w-]{16,}["']?/gi, replacement: "api_key=****" },
  ...
  { pattern: /\b[A-Z_]*(?:SECRET|PASSWORD|CREDENTIAL)[A-Z_]*\s*[:=]\s*["']?[\w/+=-]{8,}["']?/g, replacement: "****" },
  // PEM private keys
  { pattern: /-----BEGIN (?:RSA |EC )?PRIVATE KEY-----[\s\S]*?-----END (?:RSA |EC )?PRIVATE KEY-----/g, replacement: "-----REDACTED PRIVATE KEY-----" },
  // JSON credential fields
  { pattern: /"(?:password|secret|token|apiKey|api_key|access_token|refresh_token)"\s*:\s*"[^"]+"/gi, replacement: (match) => {
