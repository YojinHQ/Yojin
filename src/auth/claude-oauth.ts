...
export function buildClaudeOAuthUrl(params: {
...
export async function exchangeClaudeOAuthCode(params: {
  ...
  if (!response.ok) {
    ...
    if (!formResponse.ok) {
      ...
      throw new Error(
        `OAuth token exchange failed: ${formResponse.status} ${errorText}`,
      );
    }
    ...
    child.on("close", (code) => {
      if (capturedToken) {
        ...
        if (finalMatch) {
          ...
          reject(
            new Error(
              "setup-token completed but no token found in output",
            ),
          );
        }
      } else {
        reject(
          new Error(
            `claude setup-token exited with code ${code}. Output: ${output.slice(0, 500)}`,
          ),
        );
      }
    });
