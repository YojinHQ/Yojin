...
import type { IntegrationTier, Platform } from '../api/graphql/types.js';
...
export function createPlatformTools(connectionManager: ConnectionManager): ToolDefinition[] {
  const connectPlatform: ToolDefinition = {
    ...
    description:
      ...
      'Call with platform + tier to run the full connection flow (credentials must already be in the vault via store_credential).',
    parameters: z.object({
    ...
    async execute(params: { platform: string; tier?: string }): Promise<ToolResult> {
      ...
      if (!tier) {
        ...
        return {
          content: `Available tiers for ${platform} (best first):\n${lines.join('\n')}\n\nUse store_credential to securely collect credentials, then call connect_platform with the chosen tier.`,
        };
      }
